...
export class SecurityController {
  ...
  updateAlertStatus(
    ...
    return this.securityService.updateAlertStatus(id, updateStatusDto, null);
  }
  ...
  @ApiResponse({ status: 400, description: '请求参数错误' })
  @ApiResponse({ status: 404, description: '客户端不存在' })
  async ignoreAllAlertsPost(
    @Body() ignoreAllDto: IgnoreAllAlertsDto,
  ) {
    this.logger.log(`批量忽略违规事件请求(POST): clientId=${ignoreAllDto.clientId}`);

    try {
      const result = await this.securityService.ignoreAllAlertsForClient(ignoreAllDto.clientId, null);
      this.logger.log(`批量忽略完成(POST): clientId=${ignoreAllDto.clientId}, affected=${result.affected}`);
      return result;
    } catch (error) {
      this.logger.error(`批量忽略失败(POST): clientId=${ignoreAllDto.clientId}, error=${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async uploadScreenshot(
    ...
    try {
      ...
      const duration = Date.now() - startTime;
      this.logger.error(`截图上传失败: 客户端=${uploadDto.clientId}, 耗时=${duration}ms, 错误=${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async reportViolation(@Body() reportDto: {
    clientId: string;
    ...
    try {
      this.logger.log(`收到违规上报: 客户端=${reportDto.clientId}, 类型=${reportDto.violationType}`);

      // 创建安全告警
      const alert = await this.securityService.createSecurityAlert({
        clientId: reportDto.clientId,
        alertType: reportDto.violationType === 'BLOCKCHAIN_ADDRESS' ? AlertType.BLOCKCHAIN_ADDRESS : AlertType.SUSPICIOUS_ACTIVITY,
        blockchainAddress: reportDto.violationType === 'BLOCKCHAIN_ADDRESS' ? reportDto.violationContent : undefined,
      ...
      const duration = Date.now() - startTime;
      this.logger.log(`违规上报处理完成: 客户端=${reportDto.clientId}, 告警ID=${alert.id}, 耗时=${duration}ms`);
      ...
      const duration = Date.now() - startTime;
      this.logger.error(`违规上报失败: 客户端=${reportDto.clientId}, 耗时=${duration}ms, 错误=${error.message}`, error.stack);
      throw error;
    }
  }
}